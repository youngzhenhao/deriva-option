...
// @dev: edited
export const PaginationButtonEdited = ({ currentPage, totalItems, setCurrentPage }: PaginationButtonProps) => {
  const isPrevButtonDisabled = currentPage === 0;
  const isNextButtonDisabled = currentPage + 1 >= Math.ceil(totalItems / ITEMS_PER_PAGE);
  ...
  return (
    <div className="mt-5 justify-end flex gap-3 mx-5">
      ...
      <span className="self-center text-primary-content font-medium">页面 {currentPage + 1}</span>
      <button
        className={`btn btn-sm ${nextButtonClass}`}
        disabled={isNextButtonDisabled}
        onClick={() => setCurrentPage(currentPage + 1)}
